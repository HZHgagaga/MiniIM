        ...
        以下是当初写的设计思路，因为写的时候发现有的内容根本没法按当初瞎想的方法实现，所以实际实现的时候其实有些许不同。。。
...
一对一聊天模块（在线）：单独的UDP消息接收线程将通过循环每一个隐身和在线的好友的IP和端口号不断接收消息，并且把消息内容以追加的方式存入到消息文件中，每一个好友都将拥有独自的消息文件，若是有某个好友及其对应的IP和端口号接收到消息，在查看之前，listbox控件中对应项将增加新消息字样的标识。双击好友列表中的用户名后，将会弹出聊天窗口，此时将会读取对应的消息文件，将其显示在聊天窗口中，输入希望发送的内容，点击发送，内容将通过UDP的方式，以该用户的IP和端口号为目标，发送输入的内容，且发送的内容保存在一个长度为255的char型字符串中，并且发送的内容会以追加的方式保存到消息文件中。
